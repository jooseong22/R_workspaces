...
print(list.files()) # 현재 결로에 있는 파일목록 출력

# 엑셀(csv)파일 불러오기

emp = read.csv('emp.csv') # 엑셀 불러오기
#View(emp)
#퀴즈 -> 1~5행까지 출력, 아래 3행만 출력, 행열 수 확인
...
cat("결측치를 제거한 SAL급여 평균:", sal, "\n")
# 결측치를 제거한  SAL(급여) 총합 구하기
sal = sum(na.omit(emp$SAL))
cat("결측치를 제거한 급여 총합:", sal, "\n")
# 결측치를 제거한 COMM 표준편차 구하기
comm = sd(emp$SAL, na.rm = )
cat("결측치를 제거한 급여 총합:", sal, "\n")
